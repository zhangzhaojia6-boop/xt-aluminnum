...
- `/admin/master`、`/admin/master/templates` -> `admin-master-workshop` / `admin-template-center` -> `CommandModulePage.vue`，正式中心：主数据与模板中心；当前按高清目标图 `14-master-template.png` 对齐，使用 `masterCenterMock` fallback / mixed 只读配置底座数据展示车间、班组、员工、机台、用户、班次、别名、字典、模板配置、字段规则、owner 字段和上线试跑风险。查看车间、班组、机台、用户、模板、字段规则、刷新主数据为筛选/跳转/只读动作；导出配置、发布模板、保存字段规则在无真实接口时禁用。本页属于管理端主数据配置面，不绕过后端主数据与权限模型，不直接修改生产事实或真实模板发布。
