...
| 文件 | 解析结果 |
|---|---:|
| `2026-6-16_日报正文.txt` | 3 行 |
| `2026-6-16_日均报表.xls` | 187 行 |
| `delivery_override_2026-06-16.json` | 3 行 |
...
gstack 页面真实点击还发现一个业务日过滤问题：页面选择 `2026-6-16_日均报表.xls`、业务日填 `2026-06-16` 时，旧逻辑会把 Excel 里所有日期的 187 行都拿来对比，造成 7,146 条差异。本轮已在 `/api/v1/mapping-reconciliation/run` 解析参考文件后按 `business_date` 过滤参考行，避免跨日期混比。复验后页面显示 `109 / 336`，差异降为 1,998 条，说明跨日期混比已消除，剩余问题是车间别名和生产口径未统一。

云端 dry-run 继续暴露 3 个业务口径差异：

| 字段 | `D:\输出skill` | 系统侧 | 判断 |
|---|---:|---:|---|
| 在制料 `wip_total` | 879 | 1233068.61 | 单位或范围不一致，疑似系统侧为更大范围快照或 kg 口径 |
| 全厂用电 `total_electricity_kwh` | 168000 | 缺值 | 系统汇总行没有同口径全厂用电字段 |
| 全厂用气 `total_gas_m3` | 57776 | 4277 | 系统侧用气来源范围不完整或口径不同 |

当前结论：第一阶段“代码链路、页面、接口、测试、生产部署”已完成；“真实数据 95%+ 同口径匹配”未完成。下一小步应只处理上述 3 个生产口径，不改原始数据。
