...
- 待处理问题清单当前为空。
- 当前主线验证：`python -m pytest backend/tests -q --durations=10`，结果 `664 passed，124 deselected，30 warnings`。
- 前端源码契约验证：`python -m pytest backend/tests -m frontend_contract -q`，结果 `124 passed，664 deselected`。
