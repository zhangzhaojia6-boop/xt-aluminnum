...
### 已验证本地证据
- `python -m pytest backend/tests -q --durations=10` → **664 passed，124 deselected，30 warnings**。
- `python -m pytest backend/tests -m frontend_contract -q` → **124 passed，664 deselected**。
...
| dev | `python -m pytest backend/tests -q --durations=10` | 后端默认测试基线 | local runnable | 当前 664 passed，124 deselected，30 warnings |
| dev | `python -m pytest backend/tests -m frontend_contract -q` | 前端静态合同基线 | local runnable | 当前 124 passed，664 deselected |
