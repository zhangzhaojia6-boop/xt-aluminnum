...
- 生产只读接口探针返回 `pending_assignment_endpoint_status=200`、`pending_total=17`、`draft_entry_count=17`、`missing_machine_count=17`、`missing_shift_count=0`、`input=149.51`、`output=120.46`、`scrap=18.05`；分布为 `2050冷轧车间=9`、`精整车间=4`、`铸三车间=4`。当前 `live_factory_output=29.85` 来自 5 条 `submitted/bound` 卷级记录，待归属草稿的 `120.46t` 仍只作为待归属压力展示，不进入正式产量总数。
