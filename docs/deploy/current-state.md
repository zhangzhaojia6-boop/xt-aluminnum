...
- 2026-05-13 生产外部闸门复验：`scripts/check_statistics_module_ready.py --missing-inputs` 仍只输出 `LLM/AI 摘要增强`、`应用连接外发`、`钉钉真实人员触达` 三行，不回显真实密钥；`--json --check-live-aggregation` 返回预期 hard fail，hard issues 为 `LLM_DISABLED`、`APP_CONNECTION_DISABLED`，warning 为 `DINGTALK_NO_BOUND_USERS`。实时聚合只读探针为 `live_aggregation_business_date=2026-05-12`、`live_aggregation_total_entry_count=39`、`live_aggregation_formal_entry_count=39`、`live_aggregation_mes_row_count=23`、`live_aggregation_mes_match_count=24`、`live_aggregation_bound_to_machine_count=24`、`live_aggregation_pending_assignment_count=0`。
