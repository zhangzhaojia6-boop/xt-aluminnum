...
更新时间：2026-05-06 17:06:10 +08:00
...
- 线上部署代码的 `/api/v1/dashboard/external-readiness` 同源检查仍返回 `hard_gate_passed=False`、`module_usable=False`、`external_connection_enabled=False`，但 `MES_UNCONFIGURED` 与 `WORKFLOW_DISABLED` 已解除；当前 `hard_issue_codes=LLM_DISABLED,DINGTALK_DISABLED,APP_CONNECTION_DISABLED`。
- `/readyz` 关键状态：
