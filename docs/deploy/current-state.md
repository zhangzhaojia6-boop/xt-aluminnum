...
- 成本策略快照写入接口已补齐：`POST /api/v1/executive/cost-strategy-snapshots` 仅 admin 可用，可把 `cost_price_master / cost_workshop_strategy / cost_daily_result / cost_monthly_rollup / cost_variance_record` 表模型快照按业务唯一键 upsert；该能力承接前端策略引擎快照，不把策略快照升级为财务正式结账凭证，前端保存动作、人工复核和月度结账流程仍是后续项。
