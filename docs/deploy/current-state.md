...
- 2026-05-17 后端完全体分支完成门禁：`codex/gai` 最新 HEAD 已新增 `python scripts/check_llm_live.py --json` 和 `python scripts/dingtalk_cli.py send-test --userid <dingtalk_user_id> --json`，用于分别证明真实 LLM 回答和钉钉真实工作通知送达；本地 Windows 后端全量为 `947 passed, 3 skipped, 124 deselected`，WSL/Python 3.12 后端全量为 `947 passed, 3 skipped, 124 deselected`，GitHub Actions 对最新 HEAD 的 `frontend-build`、`backend-tests`、`compose-smoke` 全部 success，Deploy Staging 对最新 HEAD success。生产只读核查显示 MES `mvc`、workflow 和钉钉应用 token 均已配置，`token_received=true`、`token_length=32`，但 `contacts --department-id 1 --json` 仍返回 `missing_scope=qyapi_get_department_member`，生产库 `users` 与 `employees` 的 `dingtalk_user_id` 非空数量均为 0；本机加载主工作区 root `.env` 后，`python scripts/check_llm_live.py --json` 返回 `ok=true`、`response_received=true`、`DATA_HUB_LLM_OK`。因此 `docs/superpowers/plans/2026-05-16-backend-completion.md` 的代码、迁移、测试、CI、MES readiness 和本机 LLM live check 可验收，但 `钉钉推送真实送达` 仍不能打勾，不能调用 goal complete。
- 2026-05-17 已新增后端完成门禁逐项审计和聚合脚本：`docs/audits/2026-05-17-backend-completion-gate-audit.md` 记录每个完成标志的当前证据、剩余阻塞和最终验收命令；`python scripts/check_backend_completion_gate.py --json --dingtalk-userid <real_dingtalk_user_id>` 会校验计划内 readiness、LLM live check、钉钉 token 和真实工作通知送达，并保留通讯录权限诊断详情。在真实钉钉送达和聚合门禁 `ok=true` 前，目标必须保持 active。
- 2026-05-17 后端完全体已合并并部署到生产：`main@a57af67` 已通过 `./scripts/deploy_systemd_host.sh --pull http://8.140.218.13` 上线，服务器 `main...origin/main` 干净，公网 `/readyz` 返回 `status=ready`，`mes_sync.last_run_status=success`、`fetched_count=50`、`upserted_count=50`。生产 LLM 已补齐配置并通过 `PYTHONPATH=. .venv/bin/python scripts/check_llm_live.py --json`，返回 `ok=true`、`response_received=true`、`DATA_HUB_LLM_OK`；钉钉 `send-test --userid admin` 返回 `ok=true`、`detail=dingtalk_sent`；最终聚合门禁 `PYTHONPATH=. .venv/bin/python scripts/check_backend_completion_gate.py --json --dingtalk-userid admin` 返回 `ok=true`、`blockers=[]`。仍需运营跟进的非计划门禁项：`APP_CONNECTION_DISABLED`、钉钉通讯录缺 `qyapi_get_department_member`、生产 active 用户/员工 `dingtalk_user_id` 绑定数为 0。
