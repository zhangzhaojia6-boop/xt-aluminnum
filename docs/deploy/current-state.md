...
更新时间：2026-05-13 06:20:50 +08:00
...
- 管理端 `异常与补录/待归属` 已接入“草稿待归属分布”热力图：复用 `PendingAssignmentHeatmap` 和 `pendingAssignment.items`，按车间/班次展示待绑定草稿卷分布；该视图只读，不改变待归属草稿不进正式产量的边界。
