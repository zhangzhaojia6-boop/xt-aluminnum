...
- 尺寸门槛：每张高清目标图保持 `1672 x 941`。
- 体积门槛：`docs/ui-reference/highres/*.png` 跟踪文件总量保持 `<= 5.6 MB`。
- 高清目标图只作为视觉与信息架构参考，不作为静态截图嵌入产品。
- 视觉方向门槛：最新目标是深色工业科技风、蓝黑基底、冷蓝高光、少量绿色 / 橙色状态点缀。当前 `highres/*.png` 多为旧浅色历史基线，不能作为最终视觉验收证据，必须按 `IMAGE2_PROMPTS.md` 重新生成或替换。
- 覆盖门槛：`IMAGE2_PROMPTS.md` 已覆盖 16 类页面；当前 `highres/` 只有 15 张图，缺少独立的系统配置 / 数据治理完整目标图，后续补图时必须补齐。
...
- `/manage/admin/settings` 是管理端运维观测面，当前为 fallback / mixed 只读观测数据；刷新、readiness、健康检查和上线闸门仅做只读状态查看，回滚预检、导出诊断和查看日志在无真实接口时禁用，不执行部署、回滚、重启或自动修复，不伪造 health / ready / AI probe 成功。
- `/manage/admin/governance` 是管理端权限治理面，当前为 fallback / mixed 只读治理数据；查看审计、角色矩阵、风险账号和相关中心跳转仅做只读/导航动作，导出审计与保存策略在无真实接口时禁用，不绕过后端权限模型，不直接修改生产事实或真实授权策略。
- `/manage/master` 是管理端主数据配置面，当前为 fallback / mixed 只读配置底座数据；查看、筛选、跳转和刷新仅做只读/导航动作，导出配置、发布模板和保存字段规则在无真实接口时禁用，不绕过后端主数据与权限模型，不直接修改生产事实或真实模板发布。
