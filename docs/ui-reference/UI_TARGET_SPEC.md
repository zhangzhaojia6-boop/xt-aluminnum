...
- 页面背景：蓝黑基底与深石墨分层，形成全屏工业中枢，不做浅色通用后台。
- 主色：冷蓝 / 青蓝高光，用于主动作、关键线索、当前状态和数据流。
- 成功：低饱和绿色，用于已入账、已审核、服务正常。
- 警告：琥珀色，用于待审核、缺字段、口径未确认。
- 危险：克制红色，用于异常、锁定字段不一致、离谱数据。
- 文本：冷白、灰蓝和青蓝分级，数字使用等宽或 DIN 风格。

### 布局

- 桌面端使用固定侧栏 + 顶部工具条 + 内容工作区，整体接近成熟大中台驾驶舱。
- 页面内容优先表格、图表和事实链，不使用营销式 hero。
- 卡片统一为深色低圆角面板，靠边框、背景阶梯和冷蓝高光建立层级，不做卡片套卡片。
- 图表必须绑定业务问题：产量、能耗、质量、成本、库存、订单、异常、审核。
- 所有数值必须显示单位；没有可信单位时显示数据缺口，不猜。

### 动效

- 只使用克制状态动效：冷蓝数据流、审核推进、AI 读取/核对/生成。
- 禁用持续抢眼动画、廉价霓虹闪烁和 `transition: all`。
...
- `frontend/src/design/xt-tokens.css`：已有工业蓝、字号、圆角、阴影 token；需要继续补齐深色工业中枢主题 token。
...
- `docs/ui-reference/highres/*.png`：已有 15 张高清目标图，但当前多为旧的浅色基线；后续应按本规范重新生成深色工业科技风目标图。
