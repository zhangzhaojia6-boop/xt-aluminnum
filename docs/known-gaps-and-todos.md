...
- 后端已新增 `cost_monthly_review_status`，并提供月度复核状态读取与 admin-only `review/close` 状态动作；管理端策略核算页已展示“月度复核 / 复核通过 / 月结锁定”
- 后续待接入财务正式凭证、月结反审核/解锁边界、跨月差异追溯与财务角色权限；当前仍不作为财务正式结账凭证
...
- 当前 `frontend/e2e` 已有 38 个 Playwright spec 文件；质量、差异核对、日报交付等审阅中心关键流已分别落到 `quality-center.spec.js`、`reconciliation-center.spec.js`、`reports-center.spec.js`
...
- 应用连接 API 填入真实地址和密钥后，应加跑 `python scripts/check_statistics_module_ready.py --json --check-live-aggregation --check-app-connection-live`；该命令会发真实 readiness POST，只有下游返回 2xx 才能证明应用连接外发闭环
