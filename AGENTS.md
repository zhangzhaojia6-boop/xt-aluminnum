...
用户要求“下一步”“继续优化”“做减法”“增强智能体”时，在写代码前完成以下框架；不能把扫目录或读一份旧文档当成已经理解全局：

1. **校准当前状态**：核对 git 分支、HEAD、未提交改动、最近提交；生产任务再核对生产机 SHA、服务、健康检查、日志和只读数据样本。
2. **建立新鲜结构地图**：优先用 `codebase-memory-mcp` 的 `list_projects`、`index_status`、`get_architecture`、`search_graph`、`trace_path`、`detect_changes` 理解结构、调用链和改动影响。索引记录的 `head_sha` 必须等于当前 HEAD；不一致就先重建或增量更新。
3. **交叉核验事实**：Codebase Memory、CodeGraph 和 Understand 只证明代码结构，不证明生产业务正在正常运行。图谱定位后再读关键源码，并用生产 trace、数据库只读查询、接口或浏览器验证运行事实。旧 `.understand-anything` 图谱落后 HEAD 时只能作历史参考。
4. **写清真正目标**：用一句话写出用户要的业务结果、受保护主链、不可突破的事实源和权限边界。默认保护 `/entry`、`/manage`、DailyFactBundle、MES/WMS 只读链路、钉钉证据、审计和 trace。
5. **比较最多三个候选动作**：按业务价值、是否解除阻塞、是否减少系统、风险与可回滚性、能否真实验收进行比较。不要把“代码容易改”当优先级。
6. **只选一个 P0 最小闭环**：优先顺序是日报事实准确与可追溯、真实数据链路、缺失事实任务闭环、重复能力下线，最后才是新页面和视觉调整。并列时选择复用现有路由、服务、表和大仪表盘的方案。
7. **落实减量与增强**：数据中枢减入口、减重复页面、减并行口径、减过期服务；Hermes 增加理解、查证、冲突判断、主动追证、执行和闭环能力。智能体增强必须产生可审计动作，不以增加提示词或更长回答代替干活。
8. **先给用户动作指导**：开工前用小白能懂的话说明当前状态、下一步唯一 P0、为什么现在做、验收标准、失败时如何回滚，以及用户现在需要提供或执行什么。技术上能自行完成时明确写“当前无需用户动作”；只向用户索取不可推断的业务事实、凭据或不可逆决策。
9. **闭环后重新判断**：完成代码、测试、真实链路验证和改动检测后，再回到第 1 步选择下一项；不能一次并行铺开多个未验收方向。

每次给用户的下一步指导至少包含：

```text
当前状态：已经证实什么，仍缺什么证据。
下一步唯一 P0：要完成哪个最小业务闭环。
系统动作：智能体将自行做哪些事情。
用户动作：现在需要用户做什么；没有就明确写“无需动作”。
完成标准：用哪些测试、真实数据或页面结果判定完成。
```
