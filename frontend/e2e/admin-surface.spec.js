...
test('admin compatibility shortcuts land on manage modules', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '系统运维与观测' })).toBeVisible()
...
test('admin ops route renders the observability smoke surface', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/manage\/admin\/settings$/)
  await expect(page.getByTestId('manage-shell')).toBeVisible()
  await expect(opsCenter.getByRole('heading', { name: '系统运维与观测' })).toBeVisible()
  await expect(opsCenter.getByText(/healthz|readyz|响应时间/).first()).toBeVisible()
  await expect(opsCenter.getByText('错误率').first()).toBeVisible()
  await expect(opsCenter.getByText('响应时间').first()).toBeVisible()
  await expect(opsCenter.getByText('全厂正式成材率')).toBeVisible()
