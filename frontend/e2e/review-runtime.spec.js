...
test('factory route renders the production board smoke surface', async ({ page }) => {
  ...
  await expect(factoryBoard.getByText('今日产量')).toBeVisible()
...
test('cost route renders the cost benefit smoke surface', async ({ page }) => {
  ...
  const costCenter = page.getByTestId('review-cost-center')
  const detailTable = costCenter.locator('.el-table').first()

  await expect(page.getByTestId('manage-shell')).toBeVisible()
  await expect(costCenter.getByRole('heading', { name: '成本核算与效益中心' })).toBeVisible()
  await expect(costCenter.getByText('总成本')).toBeVisible()
  await expect(costCenter.getByText('AI 成本解释卡')).toBeVisible()
  await expect(costCenter.getByText('成本拆解')).toBeVisible()
  await expect(costCenter.getByRole('button', { name: '重新计算' })).toBeVisible()
  await expect(costCenter.getByRole('button', { name: '应用参数' })).toBeVisible()
  await expect(detailTable).toBeVisible()
  await expect(detailTable.getByRole('columnheader', { name: '表名' })).toBeVisible()
  await expect(detailTable.getByRole('columnheader', { name: '状态' })).toBeVisible()
...
test('brain route renders the AI control smoke surface', async ({ page }) => {
  ...
  await expect(reviewAside.locator('.xt-manage__nav-item.is-active', { hasText: 'AI 工作台' })).toBeVisible()
  await expect(reviewAside.getByRole('link', { name: '总览' })).toBeVisible()
...
test('ops reliability center route renders live dashboard surface', async ({ page }) => {
  ...
  await expect(page.getByText('全厂正式成材率')).toBeVisible()
})
...
test('review roadmap legacy path redirects to review overview', async ({ page }) => {
  ...
  await expect(page.getByTestId('manage-shell')).toBeVisible()
  await expect(page.getByTestId('review-overview-center')).toBeVisible()
})
