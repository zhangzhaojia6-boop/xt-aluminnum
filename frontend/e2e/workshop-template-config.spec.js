...
test('admin can access workshop templates and machine user consumes updated template fields', async ({ page }) => {
  ...
  const updated = {
    ...
    shift_fields: original.shift_fields,
    extra_fields: [
  ...
  try {
    ...
    await machinePage.goto(`${process.env.PLAYWRIGHT_BASE_URL || 'https://127.0.0.1'}/login?machine=XT-ZD-1`)
    await machinePage.waitForURL(/\/(mobile|entry)$/)
    await machinePage.getByTestId('mobile-go-report').click()
    await machinePage.waitForURL(/\/(mobile\/report-advanced|entry\/advanced)\//)
