...
test('active workshop canonical list matches the twelve business workshops', () => {
  assert.deepEqual(ACTIVE_WORKSHOP_NAMES, EXPECTED_ACTIVE_WORKSHOPS)
})

test('workshop normalization maps common historical names to the twelve active names', () => {
...
test('active workshop filter removes retired workshops and keeps canonical rows', () => {
  ...
  assert.deepEqual(rows.map((row) => row.workshop || row.workshop_name), ['铸锭', '园区在线'])
  assert.equal(isActiveWorkshopName('冷轧三车间'), false)
  assert.equal(isActiveWorkshopName('冷轧2050'), true)
})

test('daily report rows use the canonical twelve-workshop surface', () => {
