...
test('AgentManagementPage reads the safe overview endpoint', () => {
  ...
  assert.match(apiSrc, /dispatchDueAgentOutboxMessages/)
  ...
  assert.match(apiSrc, /api\.post\('\/agent-management\/outbox\/dispatch-due'/)
...
test('AgentManagementPage can dispatch outbox messages and inspect external logs', () => {
  ...
  assert.match(pageSrc, /dispatchDueAgentOutboxMessages/)
  ...
  assert.match(pageSrc, /handleDispatchDueOutbox/)
  assert.match(pageSrc, /loadOutboxLogs/)
  assert.match(pageSrc, /执行分发/)
  assert.match(pageSrc, /批量调度到期/)
