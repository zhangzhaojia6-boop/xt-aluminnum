...
test('live Stitch surface keeps realtime wall slots wired to aggregation and SSE state', () => {
  ...
  assert.equal(surface.businessDate, '2026-06-05')
  assert.equal(surface.marketTicker.find((item) => item.label === '全厂包装')?.value, '126.42 吨')
  assert.equal(surface.marketTicker.find((item) => item.label === '成品入库')?.value, '120.5 吨')
  ...
  assert.equal(surface.realtimeKpiStrip[0].primaryLabel, '全厂包装')
