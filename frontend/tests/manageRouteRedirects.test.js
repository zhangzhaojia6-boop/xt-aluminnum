...
test('legacy route callers use the owner skeleton tabs', () => {
  ...
  assert.match(loginSrc, /return\s+auth\.adminSurface\s+\?\s+['"]\/admin['"]\s+:\s+['"]\/login['"]/, 'login landing should be admin-only')
  assert.doesNotMatch(loginSrc, /return\s+['"]\/manage\/today['"]/, 'login should not default password users to review tab')
