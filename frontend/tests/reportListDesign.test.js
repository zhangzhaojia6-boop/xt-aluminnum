...
test('ReportList keeps the real report archive data path', () => {
  ...
  assert.match(src, /previewTemplateDailyReport/)
  ...
  assert.match(apiSrc, /api\.post\(['"]\/reports\/template-daily\/preview['"]/)
})
...
test('ReportList uses the industrial blue responsive delivery surface', () => {
  ...
  assert.match(src, /data-testid="template-daily-preview"/)
