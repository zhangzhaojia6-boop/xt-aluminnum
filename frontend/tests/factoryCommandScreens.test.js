...
test('factory overview and flow screens call factory command store', () => {
  ...
  assert.match(overview, /fetchLiveAggregation/)
  assert.match(overview, /fetchPendingAssignmentEntries/)
  assert.match(overview, /buildLiveRealityStatus/)
  assert.match(overview, /实时数据日期/)
  assert.match(overview, /填报端上传/)
  assert.match(overview, /外部 MES 机列绑定/)
  assert.match(overview, /liveRealityStatus/)
  ...
  assert.match(overview, /missingOutputWeightSummary/)
  assert.match(overview, /missingOutputRoute/)
  assert.match(overview, /待补产出重量/)
  assert.match(overview, /补重量/)
  assert.match(overview, /tab: 'missingOutput'/)
