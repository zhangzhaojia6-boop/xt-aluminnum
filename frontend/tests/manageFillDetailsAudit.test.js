...
test('FillDetailsPage is wired to the three audit data sources', () => {
  ...
  assert.match(src, /mesGapIssueCount/)
  assert.match(src, /status_counts/)
...
test('TodayPage and WorkshopDashboardPage mount precise missing report panels', () => {
  ...
  assert.match(dashboardSrc, /mesProcessFlow/)
  assert.match(dashboardSrc, /mesProcessWeightText/)
  assert.match(dashboardSrc, /worker_name/)
  assert.match(dashboardSrc, /input_weight_tons/)
  assert.match(dashboardSrc, /output_weight_tons/)
  assert.match(dashboardSrc, /device === 'pc'/)
  assert.match(dashboardSrc, /let loadRequestId = 0/)
  assert.match(dashboardSrc, /requestId !== loadRequestId/)
