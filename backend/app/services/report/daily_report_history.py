...
def archive_daily_report(
    ...
    *,
    ...
    source_summary = {
        ...
        "source_status": _source_status(db, source_snapshot),
...
def _source_status(db: Session, source_snapshot: DailyFactBundleSnapshot) -> dict[str, Any]:
    ...
    if source_snapshot.run_id is None:
        return {}
    run = db.get(DailyFactBundleRun, source_snapshot.run_id)
    if run is None or not isinstance(run.source_status, dict):
        return {}
    return run.source_status
