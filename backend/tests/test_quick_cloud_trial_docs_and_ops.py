...
def test_release_freeze_checklist_requires_clean_worktree_and_github_remote() -> None:
    ...
    assert '`python -m pytest backend/tests -q --durations=10` → `664 passed，124 deselected，30 warnings`' in source
    assert '`python -m pytest backend/tests -m frontend_contract -q` → `124 passed，664 deselected`' in source
...
def test_current_deploy_state_tracks_current_head_and_validation_evidence() -> None:
    ...
    assert '`python -m pytest backend/tests -q --durations=10`：664 passed，124 deselected，30 warnings' in state
    assert '`python -m pytest backend/tests -m frontend_contract -q`：124 passed，664 deselected' in state
    ...
    assert '664 passed，124 deselected，30 warnings' in audit
    assert '124 passed，664 deselected' in audit
...
def test_api_and_cli_lane_docs_match_current_identity_boundaries() -> None:
    ...
    for token in [
        ...
        '664 passed，124 deselected，30 warnings',
        '浏览器 / 钉钉',
