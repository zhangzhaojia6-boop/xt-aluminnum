...
def test_release_freeze_checklist_requires_clean_worktree_and_github_remote() -> None:
    ...
    assert '`python -m pytest backend/tests -q --durations=10` → `666 passed，124 deselected，30 warnings`' in source
    assert '`python -m pytest backend/tests -m frontend_contract -q` → `124 passed，666 deselected`' in source
    assert '`npm --prefix frontend test` → `111 passed`' in source
...
def test_current_deploy_state_tracks_current_head_and_validation_evidence() -> None:
    ...
    assert '`python -m pytest backend/tests -q --durations=10`：666 passed，124 deselected，30 warnings' in state
    assert '`python -m pytest backend/tests -m frontend_contract -q`：124 passed，666 deselected' in state
    ...
    assert '666 passed，124 deselected，30 warnings' in audit
    assert '124 passed，666 deselected' in audit
    assert '111 passed' in audit
...
def test_api_and_cli_lane_docs_match_current_identity_boundaries() -> None:
    ...
    for token in [
        ...
        '666 passed，124 deselected，30 warnings',
        '浏览器 / 钉钉',
