...
def test_missing_report_workbook_contains_detailed_rows_and_summary() -> None:
    ...
    assert workbook.sheetnames == ['缺报明细', '车间汇总']
    detail = workbook['缺报明细']
    assert detail['A1'].value == '缺报明细 2026-05-06'
    assert detail['H4'].value == '问题类型'
    assert detail['B5'].value == '2050冷轧车间'
    assert detail['D5'].value == '主操甲'
    assert detail['F5'].value == '2026-05-06 09:30:00'
    assert detail['G5'].value == 'RA260506001'
    assert detail['H5'].value == 'MES抓到但机列未填'
    assert detail['J5'].value == 1
    assert detail['K5'].value == '1#轧机'

    summary = workbook['车间汇总']
    assert summary['A3'].value == '车间'
    assert summary['A4'].value == '2050冷轧车间'
    assert summary['B4'].value == 1
    assert summary['C4'].value == 1
    assert summary['E4'].value == 1
    assert summary['G4'].value == '主操甲'
...
def test_missing_report_workbook_appends_mes_gap_sheet() -> None:
    ...
    assert workbook.sheetnames == ['缺报明细', '车间汇总', 'MES异常明细']
