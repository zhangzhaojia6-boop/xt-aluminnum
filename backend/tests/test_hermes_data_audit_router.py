...
def test_get_hermes_data_audit_run_ignores_dingtalk_source_errors_in_decision_gate(monkeypatch) -> None:
    monkeypatch.setenv('HERMES_DATA_AUDIT_APPLY_ENABLED', 'true')
    engine = _make_engine()
    Base.metadata.create_all(engine, tables=ROUTER_TABLES)
    db = Session(engine)
    run = HermesDataAuditRun(
        run_key='run-dingtalk-source-error-ignored',
        business_date=date(2026, 6, 18),
        status='completed',
        source_status={
            'mes': 'ok',
        ...
        suggested_actions=[
            {
                'idempotency_key': 'dingtalk-ignored:1',
                'action_type': 'mapping_alias_upsert',
                'risk_level': 'low',
                'target_table': 'master_code_aliases',
                'target_key': 'workshop:精整',
...
def test_get_hermes_data_audit_run_blocks_apply_for_partial_failed_run(monkeypatch) -> None:
    monkeypatch.setenv('HERMES_DATA_AUDIT_APPLY_ENABLED', 'true')
    engine = _make_engine()
    Base.metadata.create_all(engine, tables=ROUTER_TABLES)
    db = Session(engine)
    run = HermesDataAuditRun(
        run_key='run-partial-failed-residual-suggestion',
        business_date=date(2026, 6, 18),
        status='correction_partial_failed',
        source_status={'mes': 'ok', 'hub': 'ok', 'output_skill': 'ok'},
        source_errors={},
        mes_snapshot={'records_count_by_source': {'stock_records': 3}},
        hub_snapshot={'field_count': 1},
        ...
        suggested_actions=[
            {
                'idempotency_key': 'suggested-after-partial-failed:1',
                'action_type': 'mapping_alias_upsert',
                'risk_level': 'low',
                'target_table': 'master_code_aliases',
                'target_key': 'workshop:拉矫',
                'before_value': {'entity_type': 'workshop', 'canonical_code': 'cold-roll-1850', 'alias_code': '拉矫车间'},
                'after_value': {
                    'entity_type': 'workshop',
                    'canonical_code': 'cold-roll-1850',
                    'alias_code': '拉矫',
                    'alias_name': '拉矫',
                    'source_type': 'hermes',
                    'is_active': True,
                },
                'evidence': {'reason': 'partial failed run still has residual suggestion'},
                'rollback_payload': {'restore_before_value': {'alias_code': '拉矫车间'}},
            }
        ],
        match_rate=Decimal('0.5000'),
